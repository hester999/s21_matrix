...
START_TEST(test_s21_remove_matrix) {
    matrix_t m;
    s21_create_matrix(3, 3, &m); // Предполагается, что функция вернёт 0 при успешной инициализации
    s21_remove_matrix(&m);
...
Suite *s21_matrix_suite() {
    ...
    tcase_add_test(tc_s21_remove_matrix, test_s21_remove_matrix); // Assuming your test function is named test_s21_remove_matrix
